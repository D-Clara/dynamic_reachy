# Function from the repository github : https://github.com/pollen-robotics/reachy_sdk_server
...
from get_traj import is_saving, add_goal, save_goal
...
class ReachySDKServer(Node,
    ...
    saved_goal_positions = []
    times = []
    saved_goal_done = False
    t0 = time.time()
    ...
    def __init__(self, node_name: str, timeout_sec: float = 5, pub_frequency: float = 100) -> None:
        ...
        self.logger.info(' Starting Reachy SDK server...')  
        ...
        self.logger.info('READY TO DESTROY THE WORLD!')
    ...
    def SendFullBodyCartesianCommands(
        ...
        def bg():
            ...
            if is_saving():
                try :
                    self.saved_goal_done = True
                    t1 = time.time()
                    #self.logger.info(str(self.t0-t1))
                    self.times.append(t1 - self.t0)
                    self.t0 = t1
                    self.saved_goal_positions.append(goal_position)
                except Exception as e:
                    self.logger.info(e)
                    f = open("/home/reachy/dynamic_reachy/src/data/log_error.txt", "a")
                    f.write('-------------------------------')
                    f.write(str(e))
                    f.write('\n')
                    f.close()
                    
            elif self.saved_goal_done:
                save_goal(self.saved_goal_positions, self.times)
                self.saved_goal_done = False
                self.saved_goal_positions = []
                self.times = []
