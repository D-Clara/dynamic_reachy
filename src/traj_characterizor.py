...
class TrajectoryCharacterizor:
    ...
    def __fit_polynomial(self):
        self.coeffs = np.polyfit(self.X, self.Y, self.Z, self.ply_deg)
        self.ffit = np.poly2d(self.coeffs)
        self.fderiv = self.ffit.deriv()
        return self.coeffs
    ...
    def plot_fitted_curve(self):
        ...
        ax.plot(self.X, self.Y, self.ffit([self.X, self.Y]), '-', label='FFunc')
        minax = min([min(self.X), min(self.Y), min(self.Z)])
        manax = max([max(self.X), max(self.Y), max(self.Z)])
        manax += 0.1
        minax -= 0.1
    ...
    def process(self):
        ...
        velocity = self.fderiv(release_point[0], release_point[1])
