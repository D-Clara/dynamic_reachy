...
def actif_angles_correction_empty_hand(shoulder_angle, elbow_angle, init_torque_shoulder=1.5, init_torque_elbow=0.5):
    ...
    m_forearm = 0.6
    m_arm = 0.6
    shoulder_torque =init_torque_shoulder -g*(m_arm*l_arm/2*np.sin(shoulder_angle*np.pi/180)+m_forearm*l_forearm/2*np.sin((shoulder_angle+elbow_angle)*np.pi/180)+m_forearm*l_arm*np.sin(shoulder_angle*np.pi/180)) 
    elbow_torque =init_torque_elbow -g*m_forearm*l_forearm/2*np.sin(elbow_angle*np.pi/180)
